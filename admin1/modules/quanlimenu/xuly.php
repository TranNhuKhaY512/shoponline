<?php
    ...
    if(isset($_POST['themdanhmuc'])){
        ...
        header('location:../../index.php?action=quanlimenu&query=them'); //quay lại trang them.php
    }elseif(isset($_POST['suadanhmuc'])){
        //sua
        $sql_update = "UPDATE tbl_danhmuc SET tendanhmuc='".$tenloaisp."',thutu='".$thutu."' WHERE id_danhmuc='$_GET[iddanhmuc]'";
        mysqli_query($mysqli,$sql_update);
        header('location:../../index.php?action=quanlimenu&query=lietke'); //quay lại trang lietke.php
    }else{
        //xóa
        $id=$_GET['iddanhmuc'];
        $sql_xoa = "DELETE FROM tbl_danhmuc WHERE id_danhmuc='".$id."'";
        mysqli_query($mysqli,$sql_xoa);
         header('location:../../index.php?action=quanlimenu&query=lietke');
    }
