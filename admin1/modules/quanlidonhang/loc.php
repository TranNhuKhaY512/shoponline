...
$sql_lietke_sp = "SELECT tbl_giohang.*, tbl_khachhang.* 
                  FROM tbl_giohang
                  LEFT JOIN tbl_khachhang ON tbl_giohang.id_khachhang = tbl_khachhang.id_khachhang
                  WHERE DATE(tbl_giohang.time) BETWEEN '$ngayBatDau' AND '$ngayKetThuc'
...
<td><?php echo $i ?></td>
    ...
    <?php if($row['cart_status'] == 1){
        echo '<a href="modules/quanlidonhang/xuly.php?code='.$row['code_cart'].'&status=moi"><button class="btn btn-primary">Đơn hàng mới</button></a>';
    } elseif($row['cart_status'] == 0){
        echo '<a href="modules/quanlidonhang/xuly.php?code=' . $row['code_cart'] . '&status=danggiao"><button class="btn btn-warning">Đang giao</button></a>';
    } else {
...
<th>
     ...
     <th>
      <a href="modules/quanlidonhang/xuly.php?idcart=<?php  echo $row['code_cart']; ?>">Xóa</a> </th>
